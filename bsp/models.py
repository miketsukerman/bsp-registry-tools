...
class ScanConfig:
    ...
    Attributes:
        ...
        sbom_cve_check_vex_manifest_path: Path to a Yocto VEX manifest
                                         file (e.g.
                                         ``${IMAGE_NAME}.rootfs.json``,
                                         generated by the Yocto
                                         ``vex.bbclass``) used by
                                         ``sbom-cve-check`` to filter
                                         kernel CVEs that do not apply
                                         to the compiled sources.
                                         Optional; when ``None`` no VEX
                                         manifest is passed.
        sbom_cve_check_databases_dir: Directory containing pre-downloaded
                                     NVD / CVE List databases for
                                     ``sbom-cve-check``.  When ``None``
                                     the tool uses its default location
                                     (``$SBOM_CVE_CHECK_DATABASES_DIR``
                                     or ``~/.cache/sbom-cve-check``).
                                     Useful for air-gapped or CI
                                     environments.
        sbom_cve_check_extra_args: Additional command-line arguments
                                  forwarded verbatim to ``sbom-cve-check``
                                  (e.g. ``"--disable-auto-updates"``).
    ...
    sbom_cve_check_vex_manifest_path: Optional[str] = None
    sbom_cve_check_databases_dir: Optional[str] = None
    sbom_cve_check_extra_args: Optional[str] = None
