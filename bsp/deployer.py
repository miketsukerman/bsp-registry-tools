...
class ArtifactDeployer:
    ...
    def discover_index_prefixes(
        ...
        """
        List every remote prefix in the container that holds artifacts.

        A prefix qualifies when it directly contains at least one blob that is
        neither an ``index.html`` page nor excluded by
        ``IndexConfig.exclude``.  Returns the prefixes sorted newest-looking
        (lexicographically greatest) first; an empty list is returned when the
