...
def _collect_scan_overrides(args) -> dict:
    ...
    vex_manifest_path = getattr(args, "scan_vex_manifest_path", None)
    if vex_manifest_path is not None:
        overrides["sbom_cve_check_vex_manifest_path"] = vex_manifest_path
    return overrides
...
def main() -> int:
    ...
    try:
        ...
        scan_parser.add_argument(
            ...
            dest="scan_tool",
            metavar="TOOL",
            choices=["trivy", "syft+grype", "sbom-cve-check"],
            default=None,
            help="Scanner backend: trivy (default), syft+grype, or sbom-cve-check"
        )
        scan_parser.add_argument(
            "--severity",
            type=str,
        ...
        scan_parser.add_argument(
            ...
            help=(
                ...
                "syft+grype or sbom-cve-check (can be specified multiple times). "
        ...
        scan_parser.add_argument(
            "--vex-manifest-path",
            type=str,
            dest="scan_vex_manifest_path",
            metavar="PATH",
            default=None,
            help=(
                "Path to a Yocto VEX manifest file (${IMAGE_NAME}.rootfs.json, "
                "generated by the Yocto vex.bbclass). Passed to sbom-cve-check as "
                "--yocto-vex-manifest to filter inapplicable kernel CVEs."
            )
        )
        scan_parser.add_argument(
            "--dry-run",
