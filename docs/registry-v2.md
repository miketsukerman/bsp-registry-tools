...
The registry natively supports NXP HABv4 and AHAB secure-boot flows using
`vendor_overrides`, `soc_vendors`, and `$ENV{}` key injection — no schema
changes are required.

For the full guide covering registry configuration, key generation, SoC
fusing, and image verification see
