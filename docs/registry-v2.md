...
| `build_options`   | string (opt.) | Extra flags for the `docker build` command, parsed with shell quoting rules and appended before the build context (e.g. `--no-cache --network host`). Supports environment variable expansion at build time (e.g. `${BSP_REGISTRY_DOCKER_BUILD_OPTIONS}`). Can be overridden per-invocation with the CLI flag `--docker-build-options`. |
